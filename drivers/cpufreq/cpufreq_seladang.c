/*
 *  drivers/cpufreq/cpufreq_seladang.c
 *
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_SELADANG
static
#endif
struct cpufreq_governor cpufreq_gov_seladang = {
    .name                   = "seladang",
...
static void seladang_powersave_bias_init_cpu(int cpu)
{
...
static void seladang_powersave_bias_init(void)
{
    int i;
    for_each_online_cpu(i) {
	seladang_powersave_bias_init_cpu(i);
    }
}
...
/* cpufreq_seladang Governor Tunables */
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	/*
	 * For the purpose of seladang, waiting for disk IO is an
				...
				unsigned int event)
{
	...
	this_dbs_info->rate_mult = 1;
	seladang_powersave_bias_init_cpu(cpu);
	num_misses = 0;
	/*
...
static int __init cpufreq_gov_dbs_init(void)
{
    ...
    return cpufreq_register_governor(&cpufreq_gov_seladang);
}

static void __exit cpufreq_gov_dbs_exit(void)
{
    cpufreq_unregister_governor(&cpufreq_gov_seladang);
}
...
MODULE_DESCRIPTION("'cpufreq_seladang' - A dynamic cpufreq governor for "
		   "Low Latency Frequency Transition capable processors");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_SELADANG
